...
const BASE_PRICES: Record<string, number> = {
  AAPL: 227, MSFT: 415, GOOGL: 178, AMZN: 204, NVDA: 132, TSLA: 248,
  META: 568, JPM: 238, V: 290, JNJ: 152,
  SPY: 556, QQQ: 481, IWM: 209, GLD: 232, TLT: 93, XLF: 45, ARKK: 52,
  'BTC/USD': 98400, 'ETH/USD': 3640, 'SOL/USD': 178, 'BNB/USD': 621,
  'XRP/USD': 0.62, 'ADA/USD': 0.44, 'DOGE/USD': 0.17,
  ES: 5540, NQ: 19820, CL: 78, GC: 2340, ZB: 115, SI: 30,
  'EUR/USD': 1.088, 'GBP/USD': 1.272, 'USD/JPY': 149.8, 'AUD/USD': 0.648,
  'USD/CAD': 1.362, 'USD/CHF': 0.891,
};
...
export function generateQuote(symbol: string, name: string, assetClass: AssetClass): Quote {
  ...
  return {
    ...
    change,
    changePercent,
    open: parseFloat((prevClose * (1 + (Math.random() - 0.5) * 0.005)).toFixed(2)),
    ...
    marketCap: assetClass === 'stock' ? price * (Math.random() * 10e9 + 1e9) : undefined,
...
export function getAllSymbols(): { symbol: string; name: string; assetClass: AssetClass }[] {
  return (Object.entries(SYMBOLS) as [AssetClass, { symbol: string; name: string }[]][]).flatMap(
    ([assetClass, items]) => items.map((i) => ({ ...i, assetClass }))
  );
}
