...
export async function getLiveQuote(
  ...
  try {
    ...
    if ((assetClass === 'stock' || assetClass === 'etf' || assetClass === 'futures') && hasFinnhubKey()) {
    ...
    try {
      ...
      for (const cs of cryptoSymbols) {
        ...
        if (t) {
          ...
          const spread = price * 0.001;
          results.push({
            symbol: cs.symbol, name: cs.name, assetClass: 'crypto',
      ...
      cryptoSymbols.forEach((cs) => results.push(generateQuote(cs.symbol, cs.name, cs.assetClass)));
    }
  }
  ...
  if (forexSymbols.length > 0) {
    ...
    for (const s of forexSymbols) {
      ...
      if (price) {
        ...
        const spread = price * 0.0002;
        results.push({
          symbol: s.symbol, name: s.name, assetClass: 'forex',
          price,
          change: parseFloat(change.toFixed(5)),
          changePercent: parseFloat(changePercent.toFixed(3)),
          open: parseFloat(prevClose.toFixed(5)),
...
type PriceUpdateCallback = (price: number, change: number, changePercent: number) => void;
