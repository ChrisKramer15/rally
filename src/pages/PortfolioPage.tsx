...
import { ArrowLeft, PlusCircle, Trash2, TrendingUp, TrendingDown, Search } from 'lucide-react';
...
const ASSET_CLASS_COLORS: Record<AssetClass, 'blue' | 'purple' | 'yellow' | 'green' | 'indigo'> = {
  stock:   'blue',
  etf:     'purple',
  crypto:  'yellow',
  futures: 'indigo',
...
export default function PortfolioPage() {
  ...
  if (!portfolio) {
    return (
      <div className="text-center py-20 text-slate-400">
        ...
        <button onClick={() => navigate('/')} className="mt-3 text-indigo-400 hover:underline text-sm">
          ← Back to portfolios
        </button>
      </div>
    );
  ...
  const totalCost = portfolio.investments.reduce((sum, inv) => sum + inv.avgCost * inv.quantity, 0);
  const totalValue = portfolio.investments.reduce((sum, inv) => {
    const q = quotes[inv.symbol];
    return sum + (q ? q.price * inv.quantity : inv.avgCost * inv.quantity);
  }, 0);
  ...
  return (
    <div>
      {/* Back */}
      <button onClick={() => navigate('/')} className="flex items-center gap-1 text-sm text-slate-400 hover:text-white mb-5 transition-colors">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          {portfolio.description && <p className="text-sm text-slate-400 mt-0.5">{portfolio.description}</p>}
        </div>
        <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        {[
          { label: 'Total Cost Basis', value: `$${totalCost.toLocaleString(undefined, { maximumFractionDigits: 2 })}` },
          { label: 'Market Value', value: `$${totalValue.toLocaleString(undefined, { maximumFractionDigits: 2 })}` },
          {
            label: 'Total P&L',
          ...
          <div key={card.label} className="bg-slate-800 border border-slate-700 rounded-xl p-4">
            ...
            <p className={`text-lg font-bold ${(card as { color?: string }).color ?? 'text-white'}`}>{card.value}</p>
          </div>
        ))}
      </div>
      ...
      {showAddForm && (
        <div className="bg-slate-800 border border-slate-600 rounded-xl p-5 mb-6">
          <h2 className="text-base font-semibold text-white mb-4">Add Investment</h2>
          <div className="grid sm:grid-cols-3 gap-3">
            <div className="relative">
              ...
              <div className="relative">
                ...
                <input
                  value={searchQuery}
                  onChange={(e) => { setSearchQuery(e.target.value); setSelectedSymbol(''); }}
                ...
                <div className="absolute z-10 top-full mt-1 w-full bg-slate-700 border border-slate-600 rounded-lg shadow-xl overflow-hidden">
                  {filtered.map((s) => (
                    ...
                    >
                      ...
                      <span className="text-slate-400 text-xs truncate ml-2">{s.name}</span>
                    </button>
                  ))}
                </div>
              )}
          ...
          <div className="flex gap-2 mt-3">
            ...
            <button
              onClick={() => { setShowAddForm(false); setSearchQuery(''); setSelectedSymbol(''); }}
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {portfolio.investments.map((inv) => {
                  const q = quotes[inv.symbol];
                  const price = q?.price ?? inv.avgCost;
                  const marketValue = price * inv.quantity;
                  ...
                  return (
                    <tr key={inv.id} className="border-b border-slate-700/50 hover:bg-slate-700/30 transition-colors">
                      <td className="px-4 py-3">
                        <Link to={`/valuation/${encodeURIComponent(inv.symbol).replace('%2F', '--')}`} className="hover:text-indigo-300 transition-colors">
                          <span className="font-semibold text-white">{inv.symbol}</span>
                        </Link>
                        <div className="flex items-center gap-1 mt-0.5">
                          <Badge label={inv.assetClass} variant={ASSET_CLASS_COLORS[inv.assetClass]} />
                      ...
                      <td className="px-4 py-3 text-right text-slate-300 font-mono">{inv.quantity}</td>
                      ...
                      <td className={`px-4 py-3 text-right font-mono ${isUp ? 'text-emerald-400' : 'text-red-400'}`}>
                        <div>{isUp ? '+' : ''}{pnl.toLocaleString(undefined, { maximumFractionDigits: 2, minimumFractionDigits: 2 })}</div>
                        <div className="text-xs opacity-75">{isUp ? '+' : ''}{pnlPct.toFixed(2)}%</div>
                      </td>
      ...
      {toRemove && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 border border-slate-600 rounded-xl p-6 max-w-sm w-full">
            <h3 className="text-base font-semibold text-white mb-2">Remove Investment?</h3>
            <p className="text-sm text-slate-400 mb-4">This will remove the investment from your portfolio.</p>
            <div className="flex gap-3">
              <button
                onClick={() => { removeInvestment(portfolio.id, toRemove); setToRemove(null); }}
