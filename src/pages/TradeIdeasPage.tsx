...
import { getLiveCandles, getLiveQuote } from "../services/marketData";
...
const BADGE_COLORS: Record<
  AssetClass,
  "blue" | "purple" | "yellow" | "indigo" | "green"
> = {
  stock: "blue",
  etf: "purple",
  crypto: "yellow",
...
async function buildIdeaPool(): Promise<TradeIdea[]> {
  ...
  for (let i = 0; i < allSymbols.length; i += chunkSize) {
    ...
    const results = await Promise.allSettled(
      chunk.map(async (s) => {
        const [quote, candles] = await Promise.all([
          getLiveQuote(s.symbol, s.name, s.assetClass),
          getLiveCandles(s.symbol, s.assetClass, 90),
        ]);
        return { s, quote, candles };
      }),
    );
    results.forEach((r, idx) => {
      ...
      if (quote === null || candles.length === 0) return;
      try {
        const val = computeValuation(s.symbol, quote.price, candles);
...
function IdeaCard({
  ...
  return (
    <div
      className={`relative border rounded-xl p-4 flex flex-col gap-3 ${borderColor} ${bgColor} bg-slate-800`}
    >
      {/* Dismiss button */}
      <button
      ...
      <div className="flex items-start gap-3 pr-5">
        <div
          ...
          onClick={() =>
            navigate(
              `/valuation/${encodeURIComponent(idea.symbol).replace("%2F", "--")}`,
            )
          }
        >
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            <Badge
              label={idea.assetClass}
              variant={BADGE_COLORS[idea.assetClass]}
            />
            ...
            <div className="flex items-center gap-1">
              ...
              <Badge
                label={idea.rec.style}
                variant={styleVariant(idea.rec.style)}
              />
            </div>
          </div>
        ...
        <div className="text-right flex-shrink-0">
          <div
            className={`text-xl font-bold ${confidenceColor(idea.rec.confidence)}`}
          >
            {idea.rec.confidence}%
          </div>
      ...
      <div className="grid grid-cols-4 gap-1.5 text-center text-xs">
        ...
        <div className="bg-slate-700/50 rounded-lg p-1.5">
          ...
          <div className="font-mono text-white font-semibold">
            ${pf(idea.rec.bracket.entry)}
          </div>
        </div>
        <div className="bg-red-900/30 rounded-lg p-1.5">
          <div className="text-slate-500 mb-0.5">Stop</div>
          <div className="font-mono text-red-300 font-semibold">
            ${pf(idea.rec.bracket.stopLoss)}
          </div>
        </div>
      ...
      <div className="flex items-center justify-between pt-1 border-t border-slate-700/50">
        <span className="text-xs text-slate-500">
          R:R 1:{idea.rec.bracket.riskRewardRatio}
        </span>
    ...
    <div>
        ...
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            {loading
              ...
              : `${pool.length} setups found across all assets · ${lastUpdated.toLocaleTimeString()}`}
          </p>
        </div>
        <button
          onClick={() => load(true)}
      ...
      <div className="flex items-start gap-2 bg-yellow-900/15 border border-yellow-800/40 rounded-xl px-4 py-2.5 mb-4">
        ...
        <p className="text-xs text-yellow-200/60">
          Algorithmic setups based on technical analysis. Not financial advice —
      ...
      {!loading && (
        <div className="flex flex-wrap gap-2 mb-5">
          ...
          {/* Style */}
          <div className="flex gap-1">
            {(["all", "scalp", "day", "swing", "position"] as const).map(
              (s) => (
                <FilterBtn
                  key={s}
          ...
          <div className="flex gap-1 flex-wrap">
            {(["all", "stock", "etf", "crypto", "forex"] as const).map((a) => (
              <FilterBtn
                ...
                onClick={() => setAssetFilter(a as typeof assetFilter)}
              >
                {a === "all" ? "All assets" : a}
              </FilterBtn>
            ))}
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-24 gap-3">
          ...
          <p className="text-sm text-slate-500">
            Scanning all symbols and computing valuations…
          </p>
        </div>
      ) : displayed.length === 0 ? (
        ...
        <>
          ...
          <p className="text-center text-xs text-slate-600 mt-6">
            Showing {displayed.length} of {pool.length} setups — dismiss any
            card to surface the next one
          </p>
        </>
      )}
    </div>
